  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-3xl">
          ...
          {items.map(item => (
            <li key={item.id} className="rounded border bg-white p-4">
              <div className="flex items-start gap-4">
                {item.photo_url && (
                  <img src={`http://localhost:3000${item.photo_url}`} alt="" className="h-20 w-20 rounded object-cover" />
                )}
                <div className="flex-1">
                  ...
                  {(item.latitude || item.longitude) && (
                    <p className="mt-1 text-xs text-gray-600">📍 {item.latitude}, {item.longitude}</p>
                  )}
                </div>
              </div>
            </li>
        ...
        {items.length === 0 && !loading && <p className="text-gray-600">No complaints yet.</p>}
      </div>
    </div>
  )
}
